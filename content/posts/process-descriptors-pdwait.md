...
So you don't just get a status int. You get the optional `__wrusage` (self + children resource usage) and `__siginfo` blocks too, same as `wait6`. The whole point is this: everything `wait6` could tell you, now keyed on the descriptor instead of a recyclable integer. `pdgetpid` and `wait6(P_PID, ...)` disappear from the reap path.
...
This spawns a child as a descriptor, waits for its exit through a `kqueue` loop (the integration payoff), then reaps the real status with `pdwait` (the 15.1 payoff), with no PID anywhere in the lifecycle. Build and run on FreeBSD 15.1.
...
Swap the child's `_exit(42)` for something that takes a signal, and you'll see the `WIFSIGNALED` branch report it, and all the `wait6` status decoding works because `pdwait` *is* `wait6` semantics keyed on the descriptor.
...
Capsicum's capability mode (`cap_enter(2)`) drops a process into a world where it can only act through descriptors it already holds, with no global namespaces. No opening paths by name, no looking up arbitrary PIDs. In that world, the *entire* PID-based process API is unavailable to you: you can't `kill(pid, ...)` a number you pulled from somewhere, because reaching a process by global identifier is exactly the ambient authority capability mode takes away.
